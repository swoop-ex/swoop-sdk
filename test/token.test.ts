...
describe('Token', () => {
  ...
  describe('#equals', () => {
    ...
    it('true on reference equality', () => {
      const token = new Token(ChainID.HmyMainnet, ADDRESS_ONE, 18)
      expect(token.equals(token)).toBe(true)
    })

    it('true even if name/symbol/decimals differ', () => {
      const tokenA = new Token(ChainID.HmyMainnet, ADDRESS_ONE, 9, 'abc', 'def')
      const tokenB = new Token(ChainID.HmyMainnet, ADDRESS_ONE, 18, 'ghi', 'jkl')
